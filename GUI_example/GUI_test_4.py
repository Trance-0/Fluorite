...
def decode():  
    ...
    if var!="":
        if newmethod=="Base16":
            result = base64.b16decode(bytes(var.encode('utf-8'))).decode('utf-8')
        elif newmethod=="Base32":
            result = base64.b32decode(bytes(var.encode('utf-8'))).decode('utf-8')
        elif newmethod=="Base64":
            result = base64.b64decode(bytes(var.encode('utf-8'))).decode('utf-8')
        elif newmethod=="Base85":
            result = base64.b85decode(bytes(var.encode('utf-8'))).decode('utf-8')
    t.insert('insert', result)
...
b1 = tk.Button(window, text='encode', width=10, height=1, command=encode)
b2 = tk.Button(window, text='decode', width=10, height=1, command=decode)
b1.place(x=30, y=90, anchor='nw')
b2.place(x=130, y=90, anchor='nw')
